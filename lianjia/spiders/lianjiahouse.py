...
from lianjia.items import FirstItemLoader, LianjiaItem
import re
...
class LianjiahouseSpider(CrawlSpider):
    name = 'lianjiahouse'
    allowed_domains = ['lianjia.com']
    start_urls = ['https://%s.lianjia.com/ershoufang/' % (settings['ZHANDIAN'])]

    rules = (
        Rule(LinkExtractor(allow=r'https://%s.lianjia.com/ershoufang/\d+.html' % settings['ZHANDIAN']), callback='parse_house', follow=True),
    )

    def parse_start_url(self, response):
        # 每个站的首页，爬取区域数
        url_navi_list=response.xpath("/html/body/div[3]/div[@class='m-filter']/div[@class='position']/dl[2]/dd/div[1]/div[1]/a/@href").extract()
    ...
    def parse_house(self, response):
        ...
        item_loader = FirstItemLoader(item=LianjiaItem(), response=response)
        item_loader.add_value('date',time.strftime('%Y-%m-%d', time.localtime()))  #
